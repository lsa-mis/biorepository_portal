class AddressesController < ApplicationController
  ...
  def create
    ...
    if @address.save
      if params[:loan_request] == "true"
        redirect_to step_four_path, notice: "New Address Added"
      else
  ...
  def update
    if @address.update(address_params)
      if params[:loan_request] == "true"
        redirect_to step_four_path, notice: "Address Updated"
      else
  ...
  def address_params
    params.require(:address).permit(:first_name, :last_name, :email, :address_line_1, :address_line_2, :address_line_3, :address_line_4, :city, :state, :zip, :country_code, :phone, :primary)
  end
end
